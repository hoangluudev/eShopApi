    ...
    public class UserSignUpValidator : AbstractValidator<UserSignUpRequestDto>
    {
        ...
        public UserSignUpValidator(IUserRepository userRepository)
        {
            ...
            RuleFor(x => x.Username)
                ...
                .Matches("^[a-zA-Z0-9_]+$").WithMessage(UsernameInvalidCharacters)
        ...
        //    .NotNull().WithMessage(DateOfBirthIsRequired)
        //    .Must(BeInPast).WithMessage(DateOfBirthMustBeInPast)
        //    .Must((dto, dob) => BeValidAge(dob, 18)).WithMessage(MinimumAgeRequired);
        ...
        {
            // Trả về false nếu username đã tồn tại, true nếu duy nhất
            return !(await _userRepository.UserExistsAsync(username));
        }
        private async Task<bool> BeUniqueEmail(string email, CancellationToken cancellationToken)
        {
            // Trả về false nếu email đã tồn tại, true nếu duy nhất
            return !(await _userRepository.EmailExistsAsync(email));
        }
        private static bool BeInPastDOB(DateOnly dateOfBirth)
        {
            return dateOfBirth < DateOnly.FromDateTime(DateTime.Today);
        }
        private static bool BeValidAgeDOB(DateOnly dateOfBirth, int minimumAge)
        {
